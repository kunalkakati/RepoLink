...
const isSafePreviewImageUrl = (value?: string) => {
...
export default function LinkPreview({ url, title }: LinkPreviewProps) {
  ...
  const previewImageUrl = preview.image
    ? isSafePreviewImageUrl(preview.image)
      ? preview.image.trim()
      : undefined
    : undefined;
    ...
    <div className="relative">
      ...
      {isExpanded &&
        ...
        createPortal(
          ...
          >
            <div
              className="relative flex max-h-[88vh] w-full max-w-2xl flex-col overflow-hidden rounded-[30px] border border-white/10 bg-slate-900/95 shadow-[0_25px_80px_-20px_rgba(0,0,0,0.75)]"
              onClick={(event) => event.stopPropagation()}
            >
              <div className="flex shrink-0 items-center justify-between border-b border-white/10 bg-slate-950/70 px-5 py-3">
                <div className="min-w-0">
                  <p className="text-sm font-semibold text-slate-100">
                    {preview.title || title}
                  </p>
                  {preview.domain && (
                    <p className="text-xs text-slate-400">{preview.domain}</p>
                  )}
                </div>
                <button
                  type="button"
