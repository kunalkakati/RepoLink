...
export function TagInput({ selectedTags, onChange, options, placeholder = "Add tags..." }: TagInputProps) {
  ...
  return (
    <div className="relative" ref={containerRef}>
      <div 
        className={`flex min-h-[48px] w-full flex-wrap items-center gap-2 rounded-2xl border bg-white px-3 py-2 text-sm transition-colors cursor-text ${isOpen ? "border-slate-400 ring-4 ring-slate-100" : "border-slate-300 hover:border-slate-400"}`}
        onClick={() => {
      ...
      >
        ...
        <input
          ...
          className="flex-1 bg-transparent outline-none min-w-[120px] py-1"
        />
        <button
          type="button"
          onClick={(e) => {
          ...
          <div className="max-h-60 overflow-y-auto p-1.5">
            {filteredOptions.length > 0 ? (
              ...
              <div className="px-3 py-4 text-center text-sm text-slate-500">
                {inputValue ? (
                  <>
                    Press <kbd className="rounded border bg-slate-100 px-1.5 py-0.5 font-mono text-xs">Enter</kbd> to add <strong>&quot;{inputValue}&quot;</strong>
                  </>
                ) : (
                  "No tags found"
                )}
